...
class IntegCommnetTrainer(CommnetTrainer):
    ...
    def update(self, agents, experience):
        ...
        q_loss, p_loss, target_q, target_q_next = self._update(agents, experience, obs_n, act_n, obs_next_n, noise_n, mask)
        return np.array([q_loss, p_loss, np.mean(target_q), np.mean(experience["rew"]), np.mean(target_q_next), np.std(target_q)])

    @tf.function
    def _update(self, agents, experience, obs_n, act_n, obs_next_n, noise_n, mask):
